...
def vacationDestinations(edges, origin, k): # Time Complexity: O(V + E), Space Complexity: O(V + E)
    #\ Build adjacency list where edges store destination and travel time
    adj = {}
    ...
    # Result set to store unique destinations reachable within time k
    reachable = []
    
    #  BFS Queue: (current_city, total_time_accumulated)
    # Using a queue because "unweighted" in terms of stopovers, 
    # but we track cumulative time.
    queue = deque([(origin, 0)])
    
    #visited_min_time tracks the best (lowest) time found to reach each city
    #because a stopover adds 1 hour, so paths matter.
